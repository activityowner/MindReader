...
Module ao_common
    ...
    Public donated As Boolean = False
    Public usedit As Boolean = False
    Sub VersionCheck(ByRef VersionpageLink As String, ByRef programname As String, ByVal programversion As String, ByRef ConfigDoc As mm.Document)
        ...
        If 1 = 0 Then
            ...
            If VersionCheckFrequency > 0 Then 'disable version check
    ...
    '--------------------------------------------------------------
    Function f_IsADashboardTopic(ByVal t As mm.Topic) As Boolean
        ...
        Debug.Print(t.BusinessTopic.BusinessTypeUri)
        If InStr(t.BusinessTopic.BusinessTypeUri, "www.gyronix.com/uri/rm/bt/rmquery") > 0 Then
            f_IsADashboardTopic = True
            Debug.Print("is a query true")
        Else
            If Not t.IsCentralTopic Then
                f_IsADashboardTopic = f_IsADashboardTopic(t.ParentTopic)
            Else
                f_IsADashboardTopic = False
                Debug.Print("is a query false")
            End If
        End If
    End Function
    Sub makelist(ByRef m_app As mm.Application)
        Dim i As Integer
        Dim m_doc As mm.Document
        Dim t1 As mm.Topic
        Dim t2 As mm.Topic
        Dim r As mm.Relationship
        m_doc = m_app.ActiveDocument
        If m_doc.Selection.Count > 1 Then
            For i = 1 To (m_doc.Selection.Count - 1)
                t1 = m_doc.Selection.Item(i)
                t2 = m_doc.Selection.Item(i + 1)
                r = t1.Relationships.AddToTopic(t2)
            Next
        End If
    End Sub
    ...
    Sub PlaySoundchirp()
        ...
        'sndPlaySound32("c:\windows\media\recycle.wav", 0)
        Trace.WriteLine("play dound disabled for mindjet 11")
    End Sub
    ...
    Function GetNewVersion(ByRef m_app As mm.Application, ByRef ProgramVersion As String, ByRef VersionMapLink As String) As Boolean
        ...
        VersionDoc.CentralTopic.Notes.Commit()
        On Error GoTo 0
        If Err.Number > 0 Then
...
x2:         Debug.Print("Could not get current version number from " & VersionMapLink) : Exit Function
    ...
    Function isdate0(ByRef d As Date) As Boolean
        ...
        End If
    End Function
    Sub donatedelay(ByVal weblink As String)
        If usedit Then
            If Not donated Then
                If weblink = "" Then weblink = "http://activityowner.com"
                Try
                    donated = True
    ...
    Function RMinstalled() As Boolean
        ...
        End If
        If Not regKey Is Nothing Then
            regKey = regKey.OpenSubKey("ResultsManager3")
        End If
    ...
    Function RMPathold() As String
        ...
        If Not regKey Is Nothing Then
            regKey = regKey.OpenSubKey("Gyronix")
        Else
            Debug.Print("Gyronix reg not found")
        End If
        If Not regKey Is Nothing Then
            regKey = regKey.OpenSubKey("ResultsManager")
        Else
            Debug.Print("ResultsManager reg not found")
        End If
        ...
        End If
        RMPathold = CType(regKey.GetValue("path"), String)
        If Not regKey Is Nothing Then regKey.Close()
    End Function
End Module
